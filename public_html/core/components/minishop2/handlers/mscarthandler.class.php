    ...
    public function status($data = array())
    {
        ...
        foreach ($this->cart as $item) {
            if (empty($item['ctx']) || $item['ctx'] == $this->ctx) {
                ...
                //ms3
                if ($product = $this->modx->getObject('modResource', array('class_key' => 'msProduct', 'id' => $item['id']))) {
                    $product_data = array_merge($product->toArray(), $item);
                    if (!$this->config['json_response']) {
                        $products_cart_html .= $this->ms2->pdoTools->parseChunk($this->cartRowTpl, $product_data);
                        $products_mcart_html .= $this->ms2->pdoTools->parseChunk($this->minicartRowTpl, $product_data);
                    } else {
        ...
        //ms3
        if (!$this->config['json_response']) {
            $status['html']['cart'] = $this->ms2->pdoTools->parseChunk($this->cartWrapTpl, array(
                'products' => $products_cart_html,
                'status' => $status
            ));
            $status['html']['minicart'] = $this->ms2->pdoTools->parseChunk($this->minicartWrapTpl, array(
                'products' => $products_mcart_html,
                'status' => $status
            ));
        }else{
            $status['products'] = $products_json;
        }
